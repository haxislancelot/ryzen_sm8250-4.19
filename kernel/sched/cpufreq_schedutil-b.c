				  ...
				  unsigned long util, unsigned long max)
{
	...
	/*
	 * Use the frequency step below if the calculated frequency is <70%
	 * higher than it.
	 */
	if (mult_frac(100, freq - h_freq, l_freq - h_freq) < 70)
		return h_freq;
...
static void sugov_get_util(struct sugov_cpu *sg_cpu)
{
	...
	sg_cpu->util = effective_cpu_util(sg_cpu->cpu, cpu_util_cfs(rq), NULL, NULL);
}
...
struct cpufreq_governor schedutilb_gov;
...
{
	return cpufreq_register_governor(&schedutilb_gov);
}
